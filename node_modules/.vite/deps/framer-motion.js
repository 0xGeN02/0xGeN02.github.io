...
function updateMotionValuesFromProps(element, next, prev) {
  ...
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (true) {
        warnOnce(nextValue.version === "10.9.1", `Attempting to mix Framer Motion versions ${nextValue.version} with 10.9.1 may not work as expected.`);
      }
...
function createAnimationsFromSequence(sequence, { defaultTransition = {}, ...sequenceTransition } = {}, scope) {
  ...
  for (let i = 0; i < sequence.length; i++) {
    ...
    const resolveValueSequence = (valueKeyframes, valueTransition, valueSequence, elementIndex = 0, numElements = 0) => {
      ...
      const { duration = defaultTransition.duration || 0.3, ease: ease2 = defaultTransition.ease || "easeOut" } = valueTransition;
      const delay2 = typeof valueTransition.delay === "function" ? valueTransition.delay(elementIndex, numElements) : valueTransition.delay || 0;
      ...
      maxDuration4 = Math.max(delay2 + duration, maxDuration4);
...
var createScopedAnimate = (scope) => {
  function scopedAnimate(valueOrElementOrSequence, keyframes2, options) {
    ...
    if (isSequence(valueOrElementOrSequence)) {
      animation = animateSequence(valueOrElementOrSequence, keyframes2);
